...
\noindent Oil and Gold are extensively traded commodities, and there are several macroeconomic factors which affect the prices, thus causing the prices of these commodities to be extremely volatile. Certain factors are also interdependent, which makes it extremely difficult to estimate the extent to which an individual factor could affect the price of a commodity. Moreover, the relationship between the factors and the prices could also vary over time. Also, understanding how the stock market, the commodity market, derivatives, work; how investors, hedgers use the fluctuating prices of stocks and commodities to make profits is complicated in itself. This makes the prediction of oil and gold prices a very complex and challenging problem.
...
\caption{Historical Gold Price - US Dollars per ounce from Jan 1, 1970 to Dec 9, 2014}
...
\caption{Historical Oil and Gold Price}
...
\hline
Model & Relative Error & Mean Abs Error & RMSE\\ \hline\hline
$ Baseline\ - Price\ Remains\ Same$ & $6.58$ & $5.93$ & $7.85$ \\ \hline
...
\noindent The following is a summary of all the models used for predicting the price of oil \\
...
$ Futures\ Prices $ & TBC & TBC & TBC \\ \hline
\end{tabular}
\end{center} 
...
\caption{Gold: The Relative Error Distribution}
...
On looking at the futures contracts data, one observation is that they converge to the actual price of commodity when it is a month ahead. Unfortunately, we found an outlier in the relative error distribution plot. On careful inspection of data, we found that some of the futures contracts data were missing or shown as 0. we handled such occurences by replacing with the value from the neighboring month and this resulted in the better plot. Ultimately, we found that this model performed better than all the previous models.
